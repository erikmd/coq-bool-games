...
- move=> a; apply: Rle_big_0_P_g. by move=> *; apply Rle0f.
- rewrite /index_enum -enumT Two_set.enum /=.
  ...
  transitivity (\rsum_(a | (a \in A_ 0 :|: A_ 1)) P a).
    by rewrite [X in _ = X](rsum_union (A_ 0) (A_ 1)) // -setI_eq0 setIC dis eqxx.
  rewrite cov -(pmf1 P).
...
have -> : D(P || Q) = \rsum_(a in A_ 0) P a * log (P a / Q a) +
  ...
  move=> _.
  case/Rle_lt_or_eq_dec: (Rle0f P a) => Pa.
    case/Rle_lt_or_eq_dec: (Rle0f Q a) => Qa.
...
have [A0_P_neq0 | A0_P_0] : {0 < P_A 0} + {0%R = P_A 0}.
  apply Rle_lt_or_eq_dec; apply: Rle_big_0_P_g => i _; by apply Rle0f.
- have [A1_Q_neq0 | A1_Q_0] : {0 < Q_A 1} + {0%R = Q_A 1}.
  ...
  + have [A0_Q__neq0 | A0_Q_0] : {0 < Q_A 0} + {0%R = Q_A 0}.
    ...
    * move/(Req_0_rmul_inv _ _ (Rle0f Q)) in A0_Q_0.
